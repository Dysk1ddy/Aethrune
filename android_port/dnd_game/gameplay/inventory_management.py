...
class InventoryManagementMixin:
    def manage_inventory(self, *, merchant_id: str | None = None, merchant_name: str | None = None) -> None:
        ...
        while True:
            ...
            if choice == 1:
                self.show_inventory()
            elif merchant_id is not None and merchant_name is not None and choice == 2:
            ...
            elif choice == (4 if merchant_id is not None and merchant_name is not None else 2):
                ...
                self.drop_items()
            elif merchant_id is not None and merchant_name is not None and choice == 7:
    ...
    def show_merchant_stock(self, merchant_id: str, merchant_name: str) -> None:
        ...
        if not stock:
            ...
            return
        for item_id in sorted(stock, key=lambda key: get_item(key).name):
    ...
    def buy_items(self, merchant_id: str, merchant_name: str) -> None:
        ...
        while True:
            ...
            choice = self.choose(
                ...
                [
                    f"{format_inventory_line(item_id, stock[item_id])} | buy {self.merchant_buy_price(merchant_id, item_id)} gp each"
    ...
    def sell_items(self, *, merchant_id: str | None = None, merchant_name: str | None = None) -> None:
        ...
        while True:
            ...
            choice = self.choose(
                ...
                [
                    f"{format_inventory_line(item_id, self.available_inventory_count(item_id))} | sell {self.merchant_sell_price(merchant_id, item_id)} gp each"
                    for item_id in item_ids
                ]
                + ["Back"],
                allow_meta=False,
